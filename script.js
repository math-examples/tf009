...
import { getInputs, getTestInputs} from './data.js';
...
async function testModel(){
    if(!testImgs)testImgs = await getTestInputs(); 
   predictImg(testImgs[testIndex%testImgs.length]);
   ++testIndex;
}
    // 5. 迁移学习下的模型预测
...
function setupListeners() {
    ...
    predict(ev.currentTarget.files[0])}, false);
    
    document.querySelector('#testModel').addEventListener("click", (ev) => {
   testModel();
   }, false);
   
    document.querySelector('#saveModel').addEventListener("click", (ev) => {
   saveModel();
   }, false);
}
